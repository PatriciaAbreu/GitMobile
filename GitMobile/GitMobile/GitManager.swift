...
//    autenticaçao do git
    func buscarRepos(user:String!){
        ...
        NSUserDefaults().setObject(complemento, forKey: "complemento")
    ...
    func buscarRepositorio(usuario: NSString) {
        ...
        self.buscarRepos(NSUserDefaults().objectForKey("usuario") as! String)
        ...
        for var i = 0; i < reposMackMobile.count; i++ {
            ...
            println("deu")
            salvarBanco(reposMackMobile[i], numero: number, repositorios: NSMutableArray(array: nomeLabel))
            
        }
        
    }
    
    func salvarBanco(nomeRep: String, numero: Int, repositorios: NSMutableArray) {
        
        let rep = RepositorioManager.sharedInstance.novoRepositorio()
        
        rep.setValue(nomeRep, forKey: "nomeRepositorio")
        rep.setValue(numero, forKey: "numero")
        rep.setValue(repositorios.componentsJoinedByString(","), forKey: "labels")
